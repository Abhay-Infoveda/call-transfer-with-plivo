...
import { Tool, ToolParameter } from '@/app/types/tool';

interface EditToolFormProps {
  tool: Tool;
  onUpdateTool: (toolData: Partial<Tool>) => void;
  onClose: () => void;
  isLoading: boolean;
}
...
const EditToolForm = ({ tool, onUpdateTool, onClose, isLoading }: EditToolFormProps) => {
  ...
  useEffect(() => {
    if (tool) {
      ...
      setBaseUrlPattern(tool.http.baseUrlPattern);
      setHttpMethod(tool.http.httpMethod);
      setParameters(tool.parameters || []);
      
      const headersArray = tool.http.headers 
        ? Object.entries(tool.http.headers).map(([key, value]) => ({ key, value }))
        : [];
